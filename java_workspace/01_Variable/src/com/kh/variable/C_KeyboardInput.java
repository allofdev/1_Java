...
public class C_KeyboardInput {
	...
	public void inputScanner1() {
		...
		// 입력받고자 하는 내용을 먼저 출력해서 입력을 유도해줘야함!!
	...
	public void inputScanner2() {
		 ...
		 * sc.nextLine() 이외의 메소드 : 버퍼에서 "엔터"는 읽어오지 않음!! (사용자가 입력한 값만을 읽어옴 == "엔터"가 남아있음)
		 */
		
		// 현재 이 시점에 버퍼에 엔터가 남아있으므로
		// 버퍼에 남아있는 엔터를 비워주는 과정을 추가하면 됨!!
		sc.nextLine(); // --> 엔터를 비워주는 역할을 함
		
		// sc.nextXXX이 뒤에 sc.nextLine() 메소드가 오는 경우 버퍼에 남아있는 "엔터"를 비워주는 과정 필요!
		...
		System.out.printf("%s님은 %d살이며, 사는곳은 %s 이고, 키는 %.1fcm입니다.\n", name, age, address, height);
	...
	public void inputScanner3() {
		...
		System.out.print("성별(M/F): ");	// Male M	Female F
		// char gender = sc.nextChar()
		// char gender = sc.nextLine()	// sc.nextLine()은 문자열이기 때문에 char변수에 저장할 수 없음
		char gender = sc.nextLine().charAt(0);	//						   "M a l e"
												//문자열로부터 문자를 추출하는 방법		0 1 2 3  -->인덱스는 0부터 시작
	 ...
	 * 
	 * - 주의할 점
	 *   > sc.nextXXX() 뒤에 sc.nextLine()
	 *     버퍼에 남은 엔터를 빼주는 과정 필수!! => sc.nextLine() 앞에 한번 더 써주면 됨!!
	 *     
	 *   > '문자' 값을 입력받기 위해서는 sc.nextLine() 메소드를 통해 문자열로 먼저 받아주고
	 *      그 뒤에 .charAt(index 문자 위치) 메소드를 통해 문자를 추출하는 과정!
