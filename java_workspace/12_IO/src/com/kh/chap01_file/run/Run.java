...
public class Run {
	...
	public static void main(String[] args) {
		...
		try {
			...
			f1.createNewFile();	// 이 메소드까지 호출해야만 실제 파일이 만들어짐!!	--> IOException 발생할 수도 있음
			...
			//폴더 먼저만들기 (폴더라는 클래스는 없음, 모두 파일 클래스로)
			File tempFolder = new File("D:\\temp");//확장자 없으면 폴더로인식
			tempFolder.mkdir();// 폴더를 만들겠다!
			
			File f3 = new File("D:\\temp\\test.txt");
			f3.createNewFile();// 파일을 만들겠다
	 ...
	 * * 자바에서 제공하는 스트림 클래스 
	 * > 통로의 사이즈 (1byte / 2byte)
	 *   - 바이트 스트림 : 1byte짜리 왔다 갔다 할 수있을 정도의 사이즈 (좁은통로)
	 *   			     입력(InputStream) / 출력(OutputStream)
	 *   
	 *   - 문자 스트림    : 2byte짜리 왔다 갔다 할 수 잇을 정도의 사이즈 (넓은통로)
	 *   			      입력(Reader) / 출력 (Writer) 
	 *   
	 * > 외부매체와 직접 연결하는 유무
